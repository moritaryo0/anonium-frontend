...
export default function SignupPage() {
  ...
  useEffect(() => {
    (async () => {
      if (iconImg && iconCrop) {
        const url = await buildCroppedPreview(iconImg, { crop_x: iconCrop.x, crop_y: iconCrop.y, crop_w: iconCrop.w, crop_h: iconCrop.h });
        if (iconPreviewCroppedUrl) URL.revokeObjectURL(iconPreviewCroppedUrl);
  ...
  // ステップ1: メールアドレスとパスワードで登録開始
  // バックエンドでは表示名はオプショナル（未指定の場合はメールアドレスのローカル部分を使用）
  // ステップ3で正式な表示名に更新する
  async function handleStep1Submit(e: React.FormEvent) {
    ...
    try {
      // メールアドレスとパスワードでサインアップを開始
      // 表示名は一時的にメールアドレスのローカル部分を使用（ステップ3で更新）
      const tempDisplayName = email.split('@')[0] || 'User';
      const res = await fetch(`${API}/api/accounts/signup/`, {
        ...
        body: JSON.stringify({
          email,
          password,
          display_name: tempDisplayName, // 一時的な表示名（オプショナル、バックエンドで自動生成も可能）
        }),
        credentials: 'include',
      });
      ...
      if (data.email_verification_required) {
        ...
        // バックエンドから返されるトークンはメールに含まれるため、ここでは保持しない
        setStep(2); // ステップ2に進む
      } else {
        // メール認証が不要な場合（開発環境など）は直接ステップ3に
        setStep(3);
      }
    } catch (err) {
  ...
  async function handleStep3Submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (iconFile) {
        ...
        if (iconCrop) {
          iconFormData.append('crop_x', String(iconCrop.x));
          iconFormData.append('crop_y', String(iconCrop.y));
          iconFormData.append('crop_w', String(iconCrop.w));
          iconFormData.append('crop_h', String(iconCrop.h));
        }
        ...
        if (!iconRes.ok) {
          // アイコンのアップロードに失敗しても、表示名の更新は成功しているので続行
  ...
  // OAuth登録処理
  async function handleOAuthSignup(provider: 'google' | 'x') {
    if (loading) return;
    setError("");
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center px-4 py-8">
          ...
          {step === 3 && (
            <div className="animate-fade-in">
              ...
              <form className="space-y-4" onSubmit={handleStep3Submit}>
                {/* Icon upload */}
                <div>
                  <label className="block text-sm mb-1">アイコン（任意）</label>
                  <input
                    ref={iconInputRef}
                    type="file"
                    accept="image/*"
                    ...
                    onChange={(e) => {
                      ...
                      if (f) {
                        if (f.size > MAX_IMAGE_SIZE) {
                          setError("画像のサイズが10MBを超えています。より小さい画像を選択してください。");
                  ...
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      {iconPreviewCroppedUrl ? (
                        <img src={iconPreviewCroppedUrl} alt="プレビュー" className="w-16 h-16 object-cover rounded-full border border-subtle" />
                      ) : (
                        <div className="w-16 h-16 rounded-full border border-subtle surface-1" />
                      )}
                      <button
                        type="button"
                      ...
                      >
                        <span className="material-symbols-rounded text-base" aria-hidden>photo_camera</span>
                      </button>
                    </div>
                    <div className="flex items-center gap-2">
                      {iconFile && (
                        <button
                          type="button"
                          className="px-3 py-1.5 rounded-md border border-subtle hover:bg-white/5"
                          onClick={() => {
                            setIconFile(null);
                            if (iconPreviewUrl) URL.revokeObjectURL(iconPreviewUrl);
                  ...
                  {iconFile && (
                    <p className="mt-1 text-xs text-subtle">アイコンを選択しました。</p>
                  )}
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setStep(2)}
                    className="flex-1 rounded-md px-4 py-2 border border-subtle hover:bg-white/5 transition-colors"
                  >
                    戻る
                  </button>
                  <button
                    type="submit"
                    className="flex-1 rounded-md px-4 py-2 bg-accent text-white hover:opacity-90 disabled:opacity-60 transition-colors"
                    disabled={loading}
                  >
                    {loading ? "登録中..." : "登録完了"}
                  </button>
      ...
      {editorOpen && iconPreviewUrl && iconImg && (
        <CropEditor
          ...
          onApply={(crop) => { setIconCrop(crop); setEditorOpen(false); }}
        />
      )}
    </div>
  );
